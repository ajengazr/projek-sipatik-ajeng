                ...
                .exceptionHandling(ex -> ex
                        ...
                        .authenticationEntryPoint((request, response, authException) -> {
                            if (request.getRequestURI().startsWith("/api/")) {
                                ...
                                response.getWriter().write("{\"timestamp\":\"" + LocalDateTime.now() +
                                        "\",\"status\":404,\"error\":\"Unauthorized\",\"message\":\"Token tidak valid atau tidak ditemukan\",\"path\":\""
                                        +
                                        request.getRequestURI() + "\"}");
                            } else {
                                // Web endpoint - redirect to login
                                response.sendRedirect("/error?status=404");
                            }
                        })
                        .authenticationEntryPoint((request, response, authException) -> {
                            if (request.getRequestURI().startsWith("/api/")) {
                                ...
                                response.getWriter().write("{\"timestamp\":\"" + LocalDateTime.now() +
                                        "\",\"status\":500,\"error\":\"Unauthorized\",\"message\":\"Token tidak valid atau tidak ditemukan\",\"path\":\""
                                        +
                                        request.getRequestURI() + "\"}");
                            } else {
                                // Web endpoint - redirect to login
                                response.sendRedirect("/error?status=500");
                            }
                        })
